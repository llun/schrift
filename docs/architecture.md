...
- ~~Offline editing/sync queue.~~ **Superseded.** Offline *reading* of
  ...
  document landed 2026-08-02, and **sub-pages of a document that is itself
  unsynced** followed on 2026-08-06 — the replay orders two dependent creates by
  refusing to POST a record whose parent is still pending, and repointing every
  such child at the id the server assigns the moment the parent lands.
- Comments/threads.
...
├── App/                  — app entry point, root navigation (RootView), appearance store. The tab shell is Features/Home/MainTabView: one system TabView (Tab(role: .search) last) with a NavigationStack per tab, the docs tab hosting HomeSplitView at regular width. The swipe-back restorer is gone — the system back button restores the gesture itself
├── DesignSystem/         — tokens ported from tokens/*.css (indigo theme), and components: Button, IconButton, Avatar, AvatarGroup, Badge, DocIcon, MaterialSymbol, SearchField, Switch, TextField, ListRow, ListSection, DocRow, LinkReachPill, ShareMemberRow, OfflineBanner, SheetHeader, Skeleton, Toast
...
- **Home** — a system large-title navigation bar (subtitle = server host, new-doc toolbar action), `SearchField`, Pinned + Recent sections of `DocRow`s — **each document renders in exactly one of the two**: the recents feed is fetched unfiltered, so `recentsExcludingPinned` subtracts whatever the Pinned section is already drawing (pinned wins, Recent is the residue; keyed on membership of the rendered pinned list, never on the `isFavorite` flag, which paginated `favorite_list/` responses make disagree — see `CLAUDE.md`) — the system `TabView` (Schrift/Shared/Profile plus the separated search role — all four tabs are fully implemented: Search hits `GET /documents/search/?q=` with recent-search history, Shared lists documents shared *with* the user (`GET /documents/?is_creator_me=false`) with an offline metadata cache, each row enriched best-effort with its members' avatars and the sharer's name from that document's `accesses/` (the "shared by me" scope was removed — the list API has no distinct by-me shared query), Profile shows the current user via `GET /users/me/` in a row that pushes the **Account screen** (`AccountScreen`, trimmed to the fields `/users/me/` actually returns; it renders an explicit unavailable state rather than a placeholder name when the user hasn't loaded), with the appearance/language pickers, server + server-version rows, sign-out, and the Work Offline toggle).
- **Editor** — one **system toolbar** in both modes (`.inline`, no navigation title, since the document title is an in-canvas header): back, then trailing Edit/Share/Options, with `editorToolbarActions` swapping **Edit** for **Done** (`finishEditing`) while editing. The save status lives in the shared document header's status slot (`SaveStatusIndicator`, driven by the unchanged `saveStatusDisplay` precedence, falling through to the reading sync caption when it is `.none`); it used to be a slim row pinned above the canvas, which had no reading-mode counterpart and appeared out of nothing on the first keystroke. Presence has one home rather than two: the shared header's `PresenceBar` draws it in **both** modes, so the Options button's editing-only count badge is gone — `presentedPeerCount` (was `presenceBadgeCount`) survives as the one rule for whether peer state is fresh enough to show at all, now applied on both surfaces (the reading one used to draw its avatars offline). The document title is a large content header (`DocsFont.title1`, `-0.02em` tracking) with `LinkReachPill` + the status slot + `PresenceBar` beneath it — `EditorDocumentHeader`, drawn identically by **both** canvases, editable only while editing — above the rendered/editable blocks; reading and editing share one style table (`EditorBlockStyle`: metrics, `blockTextAppearance`, `editorBlockDecoration`, `EditorBlockAdornment`) and one scroll anchor (`EditorScrollAnchorStore`, a content offset snapshotted at the swap — `EditorScrollTarget` only names the editing canvas's rows for its own `ScrollViewReader`), so tapping a block places a caret rather than re-flowing and re-scrolling the page — see `docs/design-system.md`; a Subpages section (hierarchy-glyph eyebrow, title-only rows); floating formatting toolbar (add block/bold/italic/bulleted list/checklist/quote/code block). The in-editor **Pages** drawer (`PagesTreeDrawer`) is back — the revised handoff specifies it: a leading slide-in tree of the document's nested subpages, opened from the toolbar, loading each level lazily through `GET documents/{id}/children/` and `DocumentChildrenCacheStore` (cache-first, so it works offline for levels already seen; "work offline" skips the network entirely, and creating a page never appends to — or caches — a level that was never loaded, which would hide the document's real children in the store the Subpages list shares). Pages created on this device are folded into the drawer's levels at **read** time (`mergedChildren`, from the session-scoped `pendingLocalDocumentsByParent`) rather than written into that shared cache, so they survive a level refetch and a view-model recreation without ever being served to the next account; the disclosure arrow follows `numchild > 0` **or** a level already held, since a locally-created page has no server bookkeeping to report one. Subpages remain reachable from the reading canvas's Subpages list as well. `AvatarGroup` renders collaborator stacks in the Shared tab's rows (`SharedRow`) and the editor's live-presence bar (`PresenceBar`); the mock's in-editor emoji reactions were not implemented.
