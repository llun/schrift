...
- **Editor** — one **system toolbar** in both modes (`.inline`, no navigation title, since the document title is an in-canvas header): back, then trailing Edit/Share/Options, with `editorToolbarActions` swapping **Edit** for **Done** (`finishEditing`) while editing. The save status moved into the editing surface as a slim row above the canvas (`SaveStatusIndicator`, driven by the unchanged `saveStatusDisplay` precedence), and presence becomes a count badge on the Options button while editing. The document title is a large content header (`DocsFont.title1`) inside the reading canvas — separate from the bar, matching the handoff — with `LinkReachPill` + a live "Synced X ago" caption beneath it, above the rendered/editable blocks; a Subpages section (hierarchy-glyph eyebrow, title-only rows); floating formatting toolbar (add block/bold/italic/bulleted list/checklist/quote/code block). The in-editor **Pages** drawer (`PagesTreeDrawer`) is back — the revised handoff specifies it: a leading slide-in tree of the document's nested subpages, opened from the toolbar, loading each level lazily through `GET documents/{id}/children/` and `DocumentChildrenCacheStore` (cache-first, so it works offline for levels already seen; "work offline" skips the network entirely, and creating a page never appends to — or caches — a level that was never loaded, which would hide the document's real children in the store the Subpages list shares). Pages created on this device are folded into the drawer's levels at **read** time (`mergedChildren`, from the session-scoped `pendingLocalDocumentsByParent`) rather than written into that shared cache, so they survive a level refetch and a view-model recreation without ever being served to the next account; the disclosure arrow follows `numchild > 0` **or** a level already held, since a locally-created page has no server bookkeeping to report one. Subpages remain reachable from the reading canvas's Subpages list as well. `AvatarGroup` renders collaborator stacks in the Shared tab's rows (`SharedRow`) and the editor's live-presence bar (`PresenceBar`); the mock's in-editor emoji reactions were not implemented.
