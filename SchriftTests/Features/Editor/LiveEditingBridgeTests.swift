...
final class LiveEditingBridgeTests: XCTestCase {
    ...
    func testReEngageAfterReturningToCleanReseedsAndResumes() async throws {
        ...
        provider.setProjection(try projectedDoc(fromMarkdown: "Alpha remote\n\nBeta"), for: documentID)
        bridge.replicaDidChange()
        ...
        // A new remote projection now applies (re-seeded from the post-save blocks).
        let newBlockID = viewModel.blocks[0].id
        provider.setProjection(
            try projectedDoc(fromMarkdown: "Alpha locally edited, then remote too\n\nBeta"), for: documentID)
        bridge.replicaDidChange()

        XCTAssertTrue(bridge.isEngaged, "resumes after returning to clean")
        XCTAssertEqual(viewModel.blocks.map(\.id).first, newBlockID, "reseeded against the post-save blocks")
